...
public class Planet extends UniverseObject {
    ...
    public void buildGoldFactory() {
        if (master != null && master.addGold(-Properties.properties.GOLD_PER_BUIDING)) {
    ...
    public void buildAngar() {
        if (master != null && master.addGold(-Properties.properties.GOLD_PER_ANGAR)
    ...
    public void makeShip() {
        if (has_angar && master != null && master.addGold(-Properties.properties.GOLD_PER_SHIP)
            ...
            getMaster().addShip(new SpaceShip(getX(), getY(), getMaster(), IdGenerator.getNewId()));
        }
    }
